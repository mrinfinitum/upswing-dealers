...
export default async function DealerOrganizationPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ view?: string }> }) {
  ...
  return (
    <div className="admin-page admin-organization-page">
      <Link className="admin-back-link" href="/admin/users">← Users</Link>
      ...
      <section className="admin-organization-section"><header><div><p className="eyebrow">Portal access</p><h2>Authorized dealers</h2><p>Control which pages each dealer user can access.</p></div></header>{memberships.length ? <div className="admin-membership-grid">{memberships.map((membership) => { const profile = profiles.find((candidate) => candidate.user_id === membership.user_id); return <article key={membership.user_id}><header><div><span>{organization.name}</span><h3>{profile?.display_name || profile?.email || "Dealer"}</h3><p>{profile?.email}</p></div><span className={membership.active ? "admin-status admin-status--verified" : "admin-status"}>{membership.active ? "Active" : "Paused"}</span></header><DealerMembershipForm membership={{ userId: membership.user_id, organizationId: id, permissions: membership.page_permissions, active: membership.active }} /></article>; })}</div> : <p className="admin-empty">No dealer users are assigned to this organization.</p>}</section>
    </div>
  );
}
