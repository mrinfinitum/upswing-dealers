...
export default async function DealerOrganizationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="admin-page admin-organization-page">
      <Link className="admin-back-link" href="/admin/dealers">← Dealer users</Link>
      ...
      <section className="admin-organization-section"><header><div><p className="eyebrow">Dealer network</p><h2>Locations</h2><p>Review every public and internal field, then open the full editor to make verified changes.</p></div><Link className="admin-button" href={`/admin/locations?q=${encodeURIComponent(organization.name)}`}>Search all locations</Link></header>{dealers.length ? <div className="admin-dealer-location-grid">{dealers.map((dealer) => {
