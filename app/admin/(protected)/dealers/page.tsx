...
function groupDealers(dealers: Dealer[]): DealerGroup[] {
  ...
  return [...groups.entries()].map(([name, locations]) => ({
    ...
    categories: [...new Set(locations.map((location) => location.dealerType).filter((category): category is string => Boolean(category)))].sort(),
...
export default async function DealersPage({ searchParams }: { searchParams: Promise<DealerDirectoryParams> }) {
  ...
  const categories = [...new Set(allGroups.flatMap((group) => group.categories))].sort();
  const countries = [...new Set(allGroups.flatMap((group) => group.countries))].sort();
  const needle = q.trim().toLowerCase();
  const groups = allGroups
    .filter((group) => !needle || group.name.toLowerCase().includes(needle) || group.countries.some((item) => item.toLowerCase().includes(needle)) || group.categories.some((item) => item.toLowerCase().includes(needle)))
    .filter((group) => !category || group.categories.includes(category))
    .filter((group) => !country || group.countries.includes(country))
    .sort((left, right) => {
      if (sort === "category") return (left.categories[0] || "Uncategorized").localeCompare(right.categories[0] || "Uncategorized") || left.name.localeCompare(right.name);
  ...
  const sharedParams = { q, category, country, sort };
  const filtersApplied = Boolean(q || category || country || sort !== "name");
    ...
    <div className="admin-page admin-dealers-page">
      ...
      {setupRequired ? <section className="admin-setup"><h2>Database setup required</h2><p>Apply the Supabase migration and run the dealer import before managing dealers. See <code>docs/supabase-admin.md</code>.</p></section> : <>
        <form className="admin-dealer-controls">
          <div className="admin-field admin-dealer-search"><label htmlFor="dealer-directory-search">Search</label><input id="dealer-directory-search" name="q" defaultValue={q} placeholder="Dealer, country, or category" /></div>
          <div className="admin-field"><label htmlFor="dealer-category">Category</label><select id="dealer-category" name="category" defaultValue={category}><option value="">All categories</option>{categories.map((item) => <option value={item} key={item}>{item}</option>)}</select></div>
          ...
          <div className="admin-field"><label htmlFor="dealer-sort">Sort by</label><select id="dealer-sort" name="sort" defaultValue={sort}><option value="name">Name A–Z</option><option value="category">Category A–Z</option><option value="country">Country A–Z</option><option value="locations">Most locations</option><option value="published">Most published</option></select></div>
        ...
        {view === "cards" ? <section className="admin-dealer-directory" aria-label="Dealers">
          {groups.map((group) => <Link href={`/admin/dealers/locations?dealer=${encodeURIComponent(group.name)}`} key={group.name}><span>{group.categories.join(" · ") || "Retail partner"}</span><h2>{group.name}</h2><p>{group.locations.length} {group.locations.length === 1 ? "location" : "locations"} · {group.published} published</p><small>{group.countries.join(" · ")}</small><strong>View locations →</strong></Link>)}
        </section> : <div className="admin-table-wrap admin-dealer-directory-list"><table className="admin-table"><thead><tr><th>Dealer</th><th>Category</th><th>Country</th><th>Locations</th><th>Published</th><th><span className="sr-only">Actions</span></th></tr></thead><tbody>{groups.map((group) => <tr key={group.name}><td><strong>{group.name}</strong></td><td>{group.categories.join(" · ") || "Not categorized"}</td><td>{group.countries.join(" · ")}</td><td>{group.locations.length}</td><td>{group.published}</td><td><Link href={`/admin/dealers/locations?dealer=${encodeURIComponent(group.name)}`}>View locations →</Link></td></tr>)}</tbody></table></div>}
