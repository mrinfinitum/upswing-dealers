...
import { AddUserForm } from "@/components/admin/invite-administrator-form";
  ...
  return (
    <div className="admin-page admin-users-page">
      ...
      {setupError ? <section className="admin-setup"><h2>User directory unavailable</h2><p>{setupError}</p></section> : <>
        <section className="admin-user-summary"><article><span>Total accounts</span><strong>{users.length}</strong></article><article><span>Administrators</span><strong>{administratorCount}</strong></article><article><span>Dealers</span><strong>{dealerCount}</strong></article><article><span>Pending invitations</span><strong>{pendingCount}</strong></article></section>
        <section className="admin-user-invite"><div><p className="eyebrow">Access assignment</p><h2>Add a user</h2><p>Invite a user by email or create an account directly. Dealers can be assigned to an organization with access to only the portal pages they need.</p></div><AddUserForm organizations={activeOrganizations} /></section>
