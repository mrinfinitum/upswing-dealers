  ...
  return (
    <form action={action} className="admin-user-create-form">
      <div className="admin-form-grid">
        ...
        <div className="admin-field"><label className="admin-field-label-split" htmlFor="userPassword"><span>Temporary password</span><small>At least 12 characters</small></label><input id="userPassword" name="password" type="password" minLength={12} autoComplete="new-password" required /></div>
      ...
      <button className="admin-button admin-button--primary admin-add-button" disabled={pending} type="submit">{pending ? "Creating…" : <>Create user <span aria-hidden="true">＋</span></>}</button>
    </form>
  );
}
