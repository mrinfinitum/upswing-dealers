...
const dropboxClient = readFileSync("lib/dropbox/client.ts", "utf8");
const dropboxThumbnailRoute = readFileSync("app/admin/(protected)/gallery/thumbnail/route.ts", "utf8");
const dropboxDownloadRoute = readFileSync("app/admin/(protected)/gallery/download/route.ts", "utf8");
...
assert.match(adminGalleryPage, /listDropboxImages/, "the Image Gallery reads the configured Dropbox folder");
assert.match(adminGalleryPage, /admin-gallery-thumbnail/, "the Image Gallery renders protected thumbnails");
assert.match(dropboxClient, /files\/list_folder/, "the Dropbox connector lists the configured folder recursively");
assert.match(dropboxClient, /files\/get_thumbnail_v2/, "the Dropbox connector uses the current thumbnail endpoint");
assert.match(dropboxThumbnailRoute, /getAdminIdentity/, "Dropbox thumbnails require administrator authentication");
assert.match(dropboxDownloadRoute, /getAdminIdentity/, "Dropbox downloads require administrator authentication");
